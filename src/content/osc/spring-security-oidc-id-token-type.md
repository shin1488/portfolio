...
summary: >-
  token exchange 요청에서 OidcIdToken을 subject token으로 제출하면 ID 토큰이 access token으로 라벨링되는
  버그를 Spring Security에 제기했습니다. 앞서 올린 token exchange 이슈에서 따로 재현되는 결함 하나를 갈라 낸
  것이며, 이 결함을 매핑 수정으로 덮으려는 제안들에 근거를 들어 반박했습니다.
period:
  from: "2026.07"
