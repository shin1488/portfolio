...
title: 통합 부품 관리 ERP
summary: >-
  자동차 부품사의 재고·발주·판매를 본사·지점·현장 세 화면에서 관리하는 통합 ERP 웹 서비스.
  5인 팀에서 PM을 겸해 재고 관리·AI 챗봇·인프라를 담당했습니다.
period:
  from: "2026.05"
  to: "2026.06"
...
- **해결** — 쓰기 도구는 일절 노출하지 않고 딥링크+prefill로 화면에 안내했습니다. 도구 호출에만 RFC 8693이 정의한 token exchange로 사용자 권한을 위임하고, 도구 예외와 호출 라운드는 JSON 변환과 상한으로 각각 방어했습니다.
...
public void customize(HttpRequest.Builder b, String method, URI endpoint,
    ...
    if (isToolCall(body)) {                                  // 사용자 데이터 접근
        ...
        attachBearer(b, tokenExchangeManager.authorize(      // RFC 8693 token exchange(OBO)
...
**해당 항목은 [mcp-security token exchange 샘플 기여](/osc/mcp-security-token-exchange)에 정리해 놓았습니다.**
