...
- **본사 웹** — 전사 재고 통합 대시보드, 부품·창고 마스터와 공급사 관리, 재고 관리·조정, 지점의 구매·판매 요청 현황 처리, 작업 이력 조회.
- **지점 웹** — 지점 대시보드, 구매 요청 접수와 지점 매니저 승인.
- **현장 모바일** — QR/바코드 스캔 입고, 재고 조회, 작업 이력 등 현장 재고 작업 처리.
- **AI 챗봇** — 자연어로 재고·발주를 조회해 표·차트로 답하고, 부족 재고 발주 요청서처럼 쓰기가 필요한 업무는 딥링크 + prefill로 작성 화면까지 안내.
