...
summary: >-
  ...
  온라인 굿즈 전시 사이트. 3인 팀에서 프론트엔드 전반을 단독으로 맡아 기획·구현·배포했습니다.
period:
  from: "2026.03"
  to: "2026.03"
techStack: [React 19, TypeScript, Vite, Tailwind CSS 4, Redux Toolkit, TanStack Query, Framer Motion, AWS S3]
highlights:
  - '굿즈를 4×4 그리드 선반에 배치하는 편집 UX — 데스크톱 마우스 드래그와 모바일 터치를 한 인터랙션으로 통합'
  - 'Axios 인터셉터 기반 액세스 토큰 사일런트 재발급 — 동시 401을 단일 refresh로 큐잉하고 실패 시 자동 로그아웃'
  - '편집은 로컬(Redux)에서 즉시, 저장 시점에만 서버와 위치를 diff 동기화하는 지연 반영 아키텍처'
  - 'ResizeObserver 스케일링으로 픽셀 아트 방을 데스크톱~모바일까지 일관되게 렌더'
links:
  - label: GitHub
