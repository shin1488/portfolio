...
title: 던옥 (DNFAUC)
summary: >-
  던전앤파이터 경매장 시세를 게임에 접속하지 않고 홈 화면 위젯으로 확인하는 앱.
  1인으로 기획부터 개발·출시까지 진행했습니다.
period:
  from: "2023.06"
  to: "2023.06"
...
- 던전앤파이터의 Neople Open API를 활용한 **안드로이드 홈 화면 위젯 앱**입니다. 인게임 재화·아이템은 가격 변동이 잦고 그 폭도 적지 않아, 게임에 접속하지 않고도 경매장 시세를 확인하고 싶다는 필요에서 출발했습니다.
- 사용자가 지정한 아이템의 **경매장 최저가와 평균가를 홈 화면 위젯에서 바로** 확인하고, 새로고침 버튼을 눌러 즉시 갱신할 수 있습니다.
...
메인 화면에서 아이템 이름을 입력하면 Neople Open API로 이름을 포함한 아이템 목록을 조회해, 아이콘·이름과 함께 리스트(RecyclerView, 이미지는 Glide)로 보여줍니다. 원하는 아이템을 누르면 로컬 저장소(SharedPreferences)에 저장되고, 위젯이 그 값을 읽어 시세를 표시합니다.

![아이템 검색 결과와 저장](/content/projects/dnfauc/preview2.png)

## 홈 화면 위젯

지정한 아이템의 이미지·이름을 위젯 상단에, 경매장 최저가·평균가를 하단에 표시합니다. 우측 상단의 새로고침 버튼을 눌러 현재 경매장 시세를 즉시 갱신할 수 있습니다.
...
- **네트워크 · 이미지** — Volley(Neople Open API 호출) · Glide(아이템 아이콘 로딩)
- **수익화** — Google AdMob 배너


# 위젯 구현 — 브로드캐스트 배선과 시세 데이터 흐름

---

- **문제** — 위젯은 Activity·Fragment가 아니라 `AppWidgetProvider`를 상속하며 앱 프로세스가 떠 있지 않아도 동작해야 하므로, 새로고침 버튼을 일반 View의 클릭 리스너로 처리할 수 없습니다.
- **해결** — 클릭을 `PendingIntent` 브로드캐스트로 발행하고, Provider의 `onReceive`에서 받아 시세 API 요청과 `RemoteViews` 갱신까지 이어지도록 배선했습니다.
- **결과** — 앱을 열지 않고도 홈 화면에서 새로고침 버튼을 눌러 최신 경매장 시세를 확인할 수 있습니다.

## 클릭을 브로드캐스트로

이 갱신 배선에 개발 시간의 대부분을 쏟았습니다.
...
시세는 Neople Open API 경매장 엔드포인트를 `unitPrice` 오름차순으로 정렬해 요청하고, **가장 싼 매물(`rows[0]`)의 단가를 최저가**로, **API가 함께 반환하는 `averagePrice`를 평균가**로 위젯에 표시합니다(평균은 앱이 계산하지 않고 API 값을 그대로 사용합니다).
...
```java
// 검색 결과 클릭(ResultAdapter) — 선택한 아이템을 로컬에 저장
SharedPreferences.Editor editor = itemView.getContext()
        .getSharedPreferences("SavedItem", Context.MODE_PRIVATE).edit();
editor.clear();
editor.putString("itemId", itemList.get(position).getItemId());

// 위젯(AuctionWidget) — 저장된 아이템을 읽어 시세 조회에 사용
private String getItemId(Context context) {
    return context.getSharedPreferences("SavedItem", Context.MODE_PRIVATE)
            .getString("itemId", DEFAULT_ITEM_ID);
}
```
...
- **넥슨 공식 소개** — 출시 이튿날인 2023.06.29, 던전앤파이터 공식 홈페이지 **"오늘의 던파"에 소개**되었습니다.

![오늘의 던파 소개](/content/projects/dnfauc/today-dnf.png)

- **커뮤니티 반응** — 출시 당일, 던파 커뮤니티에서 **6월 28일 등록 게시물 중 추천 1위**를 기록했습니다(추천 72 · 댓글 77).
...
Activity·Fragment만 다루다 `AppWidgetProvider`로 위젯을 만들며, 앱이 닫혀 있는 상태에서도 동작해야 하는 UI라는 다른 실행 모델을 처음 경험했습니다. 버튼 클릭을 리스너가 아니라 `PendingIntent` 브로드캐스트로 발행하고 `BroadcastReceiver`에서 받아 API 요청까지 이어 갱신하는 흐름을 구성하는 데 개발 시간의 대부분을 쏟았습니다. 일반 View 처리와는 결이 달라, "화면에 붙어 있지 않은 UI를 어떻게 살아 움직이게 할까"를 고민한 시간이었습니다.

## 만드는 것과 출시하는 것은 다르다

실제 사용자를 염두에 둔 첫 배포작이었기 때문에, 앱을 완성하는 것과 스토어에 올리는 것이 별개라는 사실을 체감했습니다. 특히 **개인정보처리방침** 작성·제출이 낯설어 막막했는데, 개인정보 포털의 작성 지원 서비스를 찾아 문서를 만들어 제출했습니다. 릴리스 뒤 실제 사용자 의견(위젯당 아이템 개수 확대, 특정 가격 도달 시 푸시 알림 등)을 들으며 처음 목표한 앱을 어떻게 키울지에 대한 아이디어를 얻은 것도 값진 경험이었습니다. (앱은 현재 Google Play에서 내려간 상태입니다.)
